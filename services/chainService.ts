...
import { createPublicClient, http, parseAbi, formatUnits, getContract, Address } from 'viem';
...
export const fetchVaultOnChain = async (address: string): Promise<VaultData> => {
  ...
  try {
    ...
    const [
      ...
      vaultContract.read.guardian().catch(() => '0x0000000000000000000000000000000000000000'),
    ...
    const assetContract = getContract({ address: assetAddr, abi: ERC20_ABI, client: { public: client } } as any) as any;
    ...
    const vaultTokenContract = getContract({ address: vaultAddress, abi: ERC20_ABI, client: { public: client } } as any) as any;
    ...
    try {
      ...
      const assetsPerShare = await vaultContract.read.convertToAssets([oneShare]);
      sharePrice = formatUnits(assetsPerShare, assetDecimals);
    } catch (e) {
      console.warn("Could not calculate share price via convertToAssets, falling back to division", e);
      if (totalSupply > 0n) {
         const price = Number(formatUnits(totalAssets, assetDecimals)) / Number(formatUnits(totalSupply, vaultDecimals));
    ...
    if (queueCalls.length > 0) {
      const queueResults = await client.multicall({ contracts: queueCalls, allowFailure: false } as any);
      
      const supplyIds = queueResults.slice(0, supplyLen).map((r: any) => r);
      const withdrawIds = queueResults.slice(supplyLen).map((r: any) => r);
      
      withdrawIds.forEach((id: string) => withdrawQueueSet.add(id.toLowerCase()));
      
      // Combine unique IDs
      allIds = Array.from(new Set([...supplyIds, ...withdrawIds])) as `0x${string}`[];
    }
    ...
    if (allIds.length > 0) {
      ...
      for (const id of allIds) {
         marketCalls.push(
            { address: MORPHO_BLUE_ADDRESS, abi: MORPHO_BLUE_ABI, functionName: 'idToMarketParams', args: [id] },
            { address: MORPHO_BLUE_ADDRESS, abi: MORPHO_BLUE_ABI, functionName: 'market', args: [id] },
            { address: MORPHO_BLUE_ADDRESS, abi: MORPHO_BLUE_ABI, functionName: 'position', args: [id, vaultAddress] }
         );
      }
      
      const results = await client.multicall({ contracts: marketCalls, allowFailure: false } as any);
      ...
      if (uniqueCollaterals.length > 0) {
         const res = await client.multicall({ contracts: tokenCalls, allowFailure: true } as any);
      ...
      for (const m of marketDataList) {
         ...
         if (totalSupplyShares > 0n) {
            suppliedAssets = (supplyShares * totalSupplyAssets) / totalSupplyShares;
         }
         ...
         if (withdrawQueueSet.has(m.id.toLowerCase())) {
             ...
             const vaultAccessible = suppliedAssets < marketLiquidity ? suppliedAssets : marketLiquidity;
             totalMarketAvailableLiquidity += vaultAccessible;
         }

         const tokenInfo = tokenMap.get(collateralToken.toLowerCase()) || { symbol: 'UNKNOWN', decimals: 18 };
    ...
    marketAllocations.forEach(alloc => {
       const allocNum = Number(formatUnits(alloc.suppliedAssetsRaw, assetDecimals));
    ...
    let weightedLLTV = 0;
    if (totalAssets > 0n) {
        const weightedRaw = Number(formatUnits(weightedLLTVNumerator, assetDecimals)) / Number(formatUnits(totalAssets, assetDecimals));
        weightedLLTV = weightedRaw / 1e18;
    }

    // Total Available Liquidity = Vault Cash + Accessible Market Liquidity
    const totalLiquidityRaw = (vaultCashRaw as bigint) + totalMarketAvailableLiquidity;
    ...
    if (error.message && error.message.includes("revert")) {
        msg = "Transaction reverted. The address might not be a valid MetaMorpho vault.";
    }
    throw new Error(msg);
  }
};
